...
parser = argparse.ArgumentParser(description='SegTracker Demo')
parser.add_argument('input_video', type=str, help='Input video filename')
parser.add_argument('--prompt', type=str, default='cars', help='Text prompt to detect objects in key-frames')
args = parser.parse_args()

video_name = args.prompt
io_args = {
    'output_mask_dir': f'./assets/{video_name}_masks', # save pred masks
    'output_pos_dir': f'./assets/{video_name}_pos', # save pred positions
...
output_pos_dir = io_args['output_pos_dir']
...
with torch.cuda.amp.autocast():
    while cap.isOpened():
        ...
        # pred_mask内の各IDの中心を求める
        obj_ids = np.unique(pred_mask)
        obj_ids = obj_ids[obj_ids!=0]
        obj_centers = {}
        for id in obj_ids:
            obj_mask = (pred_mask==id)
            obj_center = np.mean(np.where(obj_mask), axis=1)
            obj_centers[int(id)] = obj_center.tolist()
        with open(os.path.join(output_pos_dir, str(frame_idx) + '.json'), 'w') as f:
            json.dump(obj_centers, f)
...
if args.input_video[-3:]=='mp4':
    fourcc =  cv2.VideoWriter_fourcc(*"mp4v")
elif args.input_video[-3:] == 'avi':
